...
$target_dir = "upload_studenci/";
$target_file = $target_dir . basename($_FILES["up_student"]["name"]);
$uploadOk = 1;
$plik = pathinfo($target_file,PATHINFO_EXTENSION);
// Check if image file is a actual image or fake image
// Check if file already exists
if (file_exists($target_file)) {
    $_SESSION['upload_kandydaci_blad'] = "Taki plik już istnieje na serwerze, skasuj go z folderu <strong>upload_studenci</strong> i ponów próbe";
	header('Location: aktualizacja.php');
	exit;
}
// Check file size
if ($_FILES["up_student"]["size"] > 500000) {
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["up_student"]["tmp_name"], $target_file)) {
	
	$table = "studenci";
	$fieldseparator = ","; 
	$lineseparator = "\n";
	$csvfile = "upload_studenci/baza.csv";
	...
	try
		{
			...
			array_map('unlink', glob("upload_studenci/*.csv"));
			$stmt->closeCursor();
		}	
		...
		$_SESSION['upload_studenci_ok'] = "Baza została poprawnie zaktualizowana";
		header('Location: aktualizacja.php');
		exit;
    } else {
